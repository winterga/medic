...
from .TCN import TCNClassifier  # Our new TCN-only model
from .VideoFeatureDataset import VideoFeatureDataset  # from step 4
...
def load_train(params, hyper_params):
    ...
    # Create feature-based dataset instead of image-based dataset
    train_dataset = VideoFeatureDataset(
        root_dir=params['train_dir'],
        seq_length=hyper_params['seq_length'],
        sliding_step=1
    )
    valid_dataset = VideoFeatureDataset(
    ...
    # If you want a subset (like 10% for debugging)
    ...
    # Build TCN-only model with 2048 input dimension
    model = TCNClassifier(
        num_inputs=2048,
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=hyper_params['num_epochs'],
...
def train_model(params, hyper_params):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'valid']:
            ...
            running_loss = 0.0
            running_correct = 0
            total_samples = data_sizes[phase]

            with tqdm.tqdm(total=len(dataloaders[phase]), desc=f'{phase} {epoch+1}', unit='batch') as pbar:
                for feats, labels in dataloaders[phase]:
                    # feats => (N, seq_length, 2048)
                    # labels => (N,)
