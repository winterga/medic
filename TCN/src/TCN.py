...
class TCNWrapper(nn.Module):
    ...
    def forward(self, x):
        """
        x: Tensor of shape (batch, frames, channels, height, width) -> (N, T, C, H, W)
        """
        ...
        x = x.view(batch_size * seq_len, c, h, w)  # Reshape to (N*T, C, H, W)
        x = self.feature_extractor(x)  # Pass through ResNet-50 -> Outputs (N*T, feature_dim)
        
        # Reshape back to (N, T, feature_dim)
        feature_dim = x.shape[-1]  # Get extracted feature dimension
        x = x.view(batch_size, seq_len, feature_dim)  # Reshape to (N, T, feature_dim)
...
if __name__ == "__main__":
    ...
    image_height=256
    image_width=256
    x = torch.randn(batch_size, sequence_length, input_channels, image_height, image_width) # Shape NLC
    x=x.to(device)
    print(f"Allocated memory: {torch.cuda.memory_allocated() / 1024**2} MB")
    print(f"Cached memory: {torch.cuda.memory_reserved() / 1024**2} MB")
