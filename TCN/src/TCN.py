...
#     """
#     For sequence-level classification into 3 classes,
#     reading precomputed features of shape (N, L, 2048).
#     """
#     def __init__(self, num_inputs=2048, num_channels=[16,32,64], kernel_size=3, dropout=0.1, num_classes=3):
#         super().__init__()
#         self.tcn = TCN(num_inputs, num_channels, kernel_size, dropout=dropout)
...
#         """
#         x => (N, L, 2048)
#         We'll transform to (N, 2048, L) -> TCN -> (N, out_ch, L) -> pool -> (N, out_ch)
#         """
#         # transpose => (N, 2048, L)
#         x = x.transpose(1,2)
#         # => pass TCN
#         tcn_out = self.tcn(x)  # (N, final_channels, L)
#         # average pool over L
#         tcn_out = tcn_out.mean(dim=2)  # (N, final_channels)
#         logits = self.classifier(tcn_out)  # (N, num_classes)
#         return logits
...
# Import our custom CausalConv1d
from .conv import CausalConv1d

class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation, dropout, activation=nn.ReLU()):
        """
        A causal residual block with two CausalConv1d layers.
        """
        super().__init__()
        self.activation = activation
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x):
        # x => (N, in_channels, L)
        res = self.residual(x)  # => (N, out_channels, L)

        # First causal conv
        x = self.conv1(x)       # => (N, out_channels, L)
        x = self.activation(x)
        x = self.dropout(x)

        # Second causal conv
        x = self.conv2(x)       # => (N, out_channels, L)
        x = self.activation(x)
        x = self.dropout(x)

        return x + res  # same shape => no mismatch


class TCN(nn.Module):
    """
    A stack of ResidualBlocks with exponentially increasing dilation.
    """
    def __init__(self,
                 num_inputs: int,        # e.g. 2048 if your features have dim 2048
                 num_channels: ArrayLike,
                 kernel_size: int,
                 dropout: float = 0.1):
        super().__init__()
        ...
        for i in range(len(num_channels)):
            ...
            in_ch = num_inputs if i == 0 else num_channels[i - 1]
            out_ch = num_channels[i]
            block = ResidualBlock(in_ch, out_ch, kernel_size, dilation, dropout)
            layers.append(block)
...
class TCNClassifier(nn.Module):
    ...
    For sequence-level classification into 3 classes,
    reading precomputed features of shape (N, L, 2048).
    Causal TCN -> final average pooling -> linear classifier.
    """
    def __init__(self, 
                 num_inputs=2048,
    ...
    def forward(self, x):
        """
        x => (N, L, num_inputs) 
             e.g. (N, L, 2048) from your feature dataset
        Returns => (N, num_classes) 
        """
        # Transpose to (N, num_inputs, L)
        x = x.transpose(1, 2)  # => (N, 2048, L)

        # TCN => (N, final_channels, L) with causal conv
        tcn_out = self.tcn(x)

        # average pool over L => (N, final_channels)
        tcn_out = tcn_out.mean(dim=2)

        # final linear => (N, num_classes)
        logits = self.classifier(tcn_out)
        return logits
